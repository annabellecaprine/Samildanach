...
<body>
    <p>${e}</p>
        ...
        `;const i=t.querySelector("#lab-expr"),s=t.querySelector("#lab-roll"),n=t.querySelector("#lab-result"),a=t.querySelector("#lab-stats");i.oninput=()=>{const p=be.stats(i.value);p?a.innerText=`Range: ${p.min}–${p.max} | Average: ${p.average}`:a.innerText=""},i.oninput(),s.onclick=()=>{const p=i.value.trim();if(!p)return;const u=be.roll(p);u.error?n.innerHTML=`<span style="color:var(--status-error);">${k.escapeHtml(u.error)}</span>`:n.innerHTML=`
                            ...
                            `:i.map(p=>{var u;return`
                                <div class="config-card ${p.id===(s==null?void 0:s.id)?"active":""}" data-id="${p.id}">
                                    <div class="config-info">
                                        ...
                                        <div class="config-provider">${((u=me.find(v=>v.id===p.provider))==null?void 0:u.label)||Utils.escapeHtml(p.provider)} • ${Utils.escapeHtml(p.model)}</div>
                                    </div>
