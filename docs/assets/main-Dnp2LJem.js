...
<body>
    <p>${e}</p>
        ...
        `;const i=t.querySelector("#lib-sidebar"),s=t.querySelector("#lib-editor-area"),n=t.querySelector("#lib-relationships-area");if(S.allItems=await C.list(),S.entryList=new Be(i,{onSelect:a=>S.selectItem(a,s,n),onCreate:()=>S.showCreateModal()}),S.entryList.setItems(S.allItems),S.entryEditor=new Ge(s,{onSave:async a=>{await C.updateItem(a)},onNameChange:a=>{S.entryList.setItems(S.allItems)},onLinkClick:a=>{const o=S.allItems.find(r=>(r.data.name||"").toLowerCase()===a.toLowerCase());o&&S.selectItem(o,s,n)},onDelete:async a=>{await C.deleteItem(a.id),S.allItems=S.allItems.filter(o=>o.id!==a.id),S.entryList.setItems(S.allItems),S.entryEditor.showEmpty(),n.classList.add("hidden"),x.updateSession({activeEntryId:null})},getEntries:()=>S.allItems}),S.entryEditor.showEmpty(),S.relationshipManager=new We(n,{onSave:async a=>{await C.updateItem(a)},onNavigate:a=>{S.selectItem(a,s,n)}}),x.session.activeEntryId){const a=S.allItems.find(o=>o.id===x.session.activeEntryId);a&&S.selectItem(a,s,n)}},selectItem(t,e,i){S.activeItem=t,S.entryList.setActiveItem(t.id),S.entryEditor.setItem(t),i.classList.remove("hidden"),S.relationshipManager.setItem(t,S.allItems),x.updateSession({activeEntryId:t.id})},showCreateModal(){const t=document.createElement("div");t.className="grid-3",ee().forEach(i=>{const s=document.createElement("button");s.className="btn btn-secondary",s.style.cssText="flex-direction:column; padding:12px;",s.innerHTML=`<span style="font-size:20px;">${i.icon}</span><span class="text-xs">${i.label}</span>`,s.onclick=async()=>{e.close();const n=await C.addItem(i.id,{name:`New ${i.label}`,description:""});S.allItems.push(n),S.entryList.setItems(S.allItems);const a=document.querySelector("#lib-editor-area"),o=document.querySelector("#lib-relationships-area");S.selectItem(n,a,o)},t.appendChild(s)});const e=new M({title:"Create New Entry",content:t,actions:[{label:"Cancel",className:"btn btn-secondary",onClick:i=>i.close()}]});e.show()}},j={id:"grimoire",label:"Grimoire",icon:"📖",_state:{activeCategory:"item",selectedRuleId:null,searchQuery:""},render:async t=>{var h;await P.init();let e=await P.list();const i=Ne();let s=j._state.activeCategory||((h=i[0])==null?void 0:h.id)||"item",n=j._state.selectedRuleId,a=n?e.find(c=>c.id===n):null,o=j._state.searchQuery||"",r=null;async function l(){var w,m,p;e=await P.list(),a=n?e.find(u=>u.id===n):null;const c=Pe(s);let d=e.filter(u=>u.type===s);if(o){const u=o.toLowerCase();d=d.filter(b=>(b.data.name||"").toLowerCase().includes(u)||(b.data.description||"").toLowerCase().includes(u)||Object.values(b.data).some(v=>typeof v=="string"&&v.toLowerCase().includes(u)))}t.innerHTML=`
            ...
            `,t.querySelectorAll(".cat-tab").forEach(u=>{u.onclick=()=>{s=u.dataset.cat,n=null,a=null,o="",j._state.activeCategory=s,j._state.selectedRuleId=null,j._state.searchQuery="",l()}});const f=t.querySelector("#grimoire-search");if(f&&(f.oninput=u=>{o=u.target.value,j._state.searchQuery=o,l().then(()=>{const b=t.querySelector("#grimoire-search");b&&(b.focus(),b.setSelectionRange(b.value.length,b.value.length))})}),t.querySelectorAll(".rule-item").forEach(u=>{u.onclick=()=>{n=u.dataset.id,a=e.find(b=>b.id===n),j._state.selectedRuleId=n,l()}}),(w=t.querySelector("#btn-add-rule"))==null||w.addEventListener("click",async()=>{const u=await P.add(s,{name:`New ${(c==null?void 0:c.label)||"Rule"}`,description:""});console.log("[Grimoire] Created rule:",u.id),n=u.id,j._state.selectedRuleId=n,o="",j._state.searchQuery="",l()}),(m=t.querySelector("#btn-delete-rule"))==null||m.addEventListener("click",async()=>{!a||!await M.confirm("Delete Rule",`Delete "${a.data.name||"this rule"}"?`)||(await P.delete(a.id),I.show("Rule deleted","success"),n=null,a=null,j._state.selectedRuleId=null,l())}),(p=t.querySelector("#btn-save-rule"))==null||p.addEventListener("click",async()=>{var v;if(!a)return;const b={name:((v=t.querySelector("#rule-name"))==null?void 0:v.value)||""};t.querySelectorAll(".field-input").forEach(g=>{b[g.dataset.field]=g.value}),r&&(b.description=r.getValue()),a.data=b,await P.update(a),I.show("Rule saved","success"),l()}),a){const u=t.querySelector("#description-editor");u&&(r=new De(u,a.data.description||""),r.render())}}await l()}},Ee={id:"graph",label:"Graph",icon:"🕸️",render:async(t,e)=>{try{await C.init()}catch(v){t.innerHTML=`<div class="text-muted">Vault Error: ${v.message}</div>`;return}t.style.padding="0",t.innerHTML=`
        ...
        `;const i=t.querySelector("#graph-container"),s=t.querySelector("#graph-canvas"),n=s.getContext("2d"),a=()=>{s.width=i.clientWidth,s.height=i.clientHeight};a(),window.addEventListener("resize",a);const o=await C.list(),r=o.map((v,g)=>{const y=z(v.type),$=g/o.length*Math.PI*2,E=Math.min(s.width,s.height)*.3;return{id:v.id,item:v,label:v.data.name||"Untitled",icon:y.icon,color:y.color,x:s.width/2+Math.cos($)*E,y:s.height/2+Math.sin($)*E,vx:0,vy:0}}),l=Object.fromEntries(r.map(v=>[v.id,v])),h=[];o.forEach(v=>{(v.data.relationships||[]).forEach(g=>{if(l[g.targetId]){const y=re(g.type);h.push({from:v.id,to:g.targetId,label:y.label,color:y.icon})}})});let c={x:0,y:0,scale:1},d=!1,f={x:0,y:0},w=null;const m=()=>{n.clearRect(0,0,s.width,s.height),n.save(),n.translate(c.x,c.y),n.scale(c.scale,c.scale),n.lineWidth=2,h.forEach(v=>{const g=l[v.from],y=l[v.to];g&&y&&(n.strokeStyle="rgba(100, 116, 139, 0.5)",n.beginPath(),n.moveTo(g.x,g.y),n.lineTo(y.x,y.y),n.stroke())}),r.forEach(v=>{n.fillStyle=v.color||"#6366f1",n.beginPath(),n.arc(v.x,v.y,24,0,Math.PI*2),n.fill(),n.font="16px sans-serif",n.textAlign="center",n.textBaseline="middle",n.fillStyle="#fff",n.fillText(v.icon,v.x,v.y),n.font="11px sans-serif",n.fillStyle="var(--text-primary)",n.fillText(v.label,v.x,v.y+36)}),n.restore()},p=()=>{const v=s.width/2,g=s.height/2;r.forEach(y=>{r.forEach($=>{if(y.id===$.id)return;const E=y.x-$.x,F=y.y-$.y,R=Math.sqrt(E*E+F*F)||1,U=5e3/(R*R);y.vx+=E/R*U,y.vy+=F/R*U}),y.vx+=(v-y.x)*.001,y.vy+=(g-y.y)*.001}),h.forEach(y=>{const $=l[y.from],E=l[y.to];if($&&E){const F=E.x-$.x,R=E.y-$.y,U=Math.sqrt(F*F+R*R)||1,J=(U-150)*.01;$.vx+=F/U*J,$.vy+=R/U*J,E.vx-=F/U*J,E.vy-=R/U*J}}),r.forEach(y=>{w!==y&&(y.x+=y.vx*.1,y.y+=y.vy*.1),y.vx*=.9,y.vy*=.9}),m(),requestAnimationFrame(p)};p();const u=v=>({x:(v.offsetX-c.x)/c.scale,y:(v.offsetY-c.y)/c.scale}),b=(v,g)=>r.find(y=>{const $=y.x-v,E=y.y-g;return Math.sqrt($*$+E*E)<24});s.onmousedown=v=>{const g=u(v),y=b(g.x,g.y);y?w=y:(d=!0,f={x:v.clientX,y:v.clientY})},s.onmousemove=v=>{if(w){const g=u(v);w.x=g.x,w.y=g.y}else d&&(c.x+=v.clientX-f.x,c.y+=v.clientY-f.y,f={x:v.clientX,y:v.clientY})},s.onmouseup=()=>{w=null,d=!1},s.onwheel=v=>{v.preventDefault();const g=v.deltaY>0?.9:1.1;c.scale*=g,c.scale=Math.min(Math.max(.3,c.scale),3)},t.querySelector("#graph-reset").onclick=()=>{c={x:0,y:0,scale:1}},t.querySelector("#graph-relayout").onclick=()=>{r.forEach((v,g)=>{const y=g/r.length*Math.PI*2,$=Math.min(s.width,s.height)*.3;v.x=s.width/2+Math.cos(y)*$,v.y=s.height/2+Math.sin(y)*$,v.vx=0,v.vy=0})}}};class Ye{constructor(e,i,s){this.container=e,this.nodeLayer=i,this.svgLayer=s,this.nodes=[],this.links=[],this.transform={x:0,y:0,scale:1},this.isDragging=!1,this.lastMouse={x:0,y:0},this.isWiring=!1,this.wireFrom=null,this.wireLine=null,this.selectedLink=null,this.onDataChange=null}init(){this.container.onmousedown=e=>{(e.button===1||e.button===0&&e.altKey)&&(this.isDragging=!0,this.lastMouse={x:e.clientX,y:e.clientY},this.container.style.cursor="grabbing",e.preventDefault())},this.container.onmousemove=e=>{if(this.isDragging){const i=e.clientX-this.lastMouse.x,s=e.clientY-this.lastMouse.y;this.transform.x+=i,this.transform.y+=s,this.lastMouse={x:e.clientX,y:e.clientY},this.updateTransform()}if(this.isWiring&&this.wireLine){const i=this.container.getBoundingClientRect(),s=e.clientX-i.left,n=e.clientY-i.top;this.updateWirePreview(s,n)}},this.container.onmouseup=e=>{this.isDragging=!1,this.container.style.cursor="default",this.isWiring&&this.cancelWiring()},this.container.onwheel=e=>{e.preventDefault();const i=e.deltaY>0?.9:1.1;this.transform.scale*=i,this.transform.scale=Math.min(Math.max(.2,this.transform.scale),2),this.updateTransform()},window.addEventListener("keydown",e=>{e.key==="Delete"&&this.selectedLink&&(this.deleteLink(this.selectedLink),this.selectedLink=null)})}exportData(){return{nodes:this.nodes,links:this.links,transform:this.transform}}importData(e){e&&(this.nodes=[],this.links=[],this.nodeLayer.innerHTML="",e.transform&&(this.transform=e.transform,this.updateTransform()),e.nodes&&e.nodes.forEach(i=>this.addNode(i,!0)),this.links=e.links||[],this.updateLinks())}notifyChange(){this.onDataChange&&this.onDataChange(this.exportData())}updateTransform(){this.nodeLayer.style.transform=`translate(${this.transform.x}px, ${this.transform.y}px) scale(${this.transform.scale})`,this.updateLinks()}resetView(){this.transform={x:0,y:0,scale:1},this.updateTransform()}addNode(e,i=!1){this.nodes.push(e);const s=this.renderNodeElement(e);this.nodeLayer.appendChild(s),i||this.notifyChange()}renderNodeElement(e){const i=document.createElement("div");i.className="node"+(e.type?` ${e.type}`:""),i.id=e.id,i.style.left=e.x+"px",i.style.top=e.y+"px";let s=(e.inputs||[]).map(d=>`
        ...
        `;const i=t.querySelector("#lab-expr"),s=t.querySelector("#lab-roll"),n=t.querySelector("#lab-result"),a=t.querySelector("#lab-stats");i.oninput=()=>{const p=ye.stats(i.value);p?a.innerText=`Range: ${p.min}–${p.max} | Average: ${p.average}`:a.innerText=""},i.oninput(),s.onclick=()=>{const p=i.value.trim();if(!p)return;const u=ye.roll(p);u.error?n.innerHTML=`<span class="text-error">${k.escapeHtml(u.error)}</span>`:n.innerHTML=`
                ...
                `},i.onkeydown=p=>{p.key==="Enter"&&s.onclick()};const o=t.querySelector("#sim-expr"),r=t.querySelector("#sim-iterations"),l=t.querySelector("#sim-run"),h=t.querySelector("#sim-results"),c=t.querySelector("#histogram");l.onclick=()=>{const p=o.value.trim(),u=parseInt(r.value);p&&(l.disabled=!0,l.innerText="Running...",setTimeout(()=>{const b=pe.runDiceSimulation(p,u);if(l.disabled=!1,l.innerText="Run",b.error){h.classList.add("hidden"),I.show(b.error,"error");return}h.classList.remove("hidden"),t.querySelector("#stat-min").innerText=b.stats.min,t.querySelector("#stat-max").innerText=b.stats.max,t.querySelector("#stat-mean").innerText=b.stats.mean,t.querySelector("#stat-median").innerText=b.stats.median,t.querySelector("#stat-mode").innerText=b.stats.mode,t.querySelector("#stat-stddev").innerText=b.stats.stdDev;const v=pe.getHistogramData(b),g=Math.max(...v.map(y=>y.count));c.innerHTML=v.map(y=>`
                    <div class="hist-bar-container">
                        <div class="hist-bar" style="height: ${y.count/g*100}%;" title="${y.value}: ${y.count} (${y.percentage}%)"></div>
                        <span class="hist-label">${y.value}</span>
                    </div>
                `).join("")},10))};const d=t.querySelector("#cmp-expr1"),f=t.querySelector("#cmp-expr2"),w=t.querySelector("#cmp-run"),m=t.querySelector("#cmp-results");w.onclick=()=>{const p=d.value.trim(),u=f.value.trim();if(!p||!u)return;const b=pe.compare(p,u,1e3);if(b.error){m.classList.add("hidden"),I.show(b.error,"error");return}m.classList.remove("hidden"),m.innerHTML=`
...
`,p.forEach(u=>{const b=n.find(v=>v.id===u.type);if(m+=`
[${(b==null?void 0:b.label)||u.type}] ${u.data.name||"Untitled"}`,u.data.description){const v=u.data.description.replace(/<[^>]+>/g,"").substring(0,300);m+=`: ${v}`}b!=null&&b.fields&&b.fields.forEach(v=>{u.data[v.key]&&(m+=` | ${v.label}: ${u.data[v.key]}`)}),u.data.relationships&&u.data.relationships.length>0&&(m+=`
  Relationships:`,u.data.relationships.forEach(v=>{const g=s.find(E=>E.id===v.targetId),y=(g==null?void 0:g.data.name)||"(Unknown)",$=v.type||"related to";m+=`
    - ${$}: ${y}`,v.notes&&(m+=` (${v.notes})`)}))}),m+=`
[END CONTEXT]`)}return m}async function w(){const m=o.value.trim();if(!m)return;const p={id:ne(),role:"user",content:m,timestamp:new Date().toISOString()};e.history.push(p),o.value="",d(),i(),r.disabled=!0,r.textContent="Thinking...";try{if(!G.getActiveConfig())throw new Error("No API configuration. Go to Settings to add one.");const b=f(),g=e.history.map(E=>({role:E.role==="user"?"user":"model",content:E.content})).slice(-20),y=await G.generate(b,g),$={id:ne(),role:"model",content:y,timestamp:new Date().toISOString()};e.history.push($),i(),d()}catch(u){console.error("[Scribe]",u),e.history.push({id:ne(),role:"model",content:`[Error: ${u.message}]`,timestamp:new Date().toISOString()}),d()}finally{r.disabled=!1,r.textContent="Send"}}r.onclick=w,o.onkeydown=m=>{m.key==="Enter"&&!m.shiftKey&&(m.preventDefault(),w())},t.querySelector("#btn-clear").onclick=async()=>{await M.confirm("Clear Chat","Clear all messages?")&&(e.history=[],i(),d(),I.show("Chat cleared","success"))},t.querySelector("#btn-save-session").onclick=async()=>{const m=await M.prompt("Session name:",`Session ${Object.keys(e.sessions).length+1}`);m&&(e.sessions[m]={history:[...e.history],mode:e.mode,selectedEntries:[...e.selectedEntries],savedAt:new Date().toISOString()},i(),h.innerHTML='<option value="">-- Sessions --</option>'+Object.keys(e.sessions).map(p=>`<option value="${p}">${p}</option>`).join(""),I.show("Session saved","success"))},t.querySelector("#btn-load-session").onclick=()=>{const m=h.value;if(!m||!e.sessions[m])return;const p=e.sessions[m];e.history=[...p.history],e.mode=p.mode,e.selectedEntries=[...p.selectedEntries],i(),ge.render(t)}}},Ae={id:"export",label:"Export",icon:"📤",render:(t,e)=>{t.innerHTML=`
                            ...
                            `:i.map(p=>{var u;return`
                                <div class="config-card ${p.id===(s==null?void 0:s.id)?"active":""}" data-id="${p.id}">
                                    <div class="config-info">
                                        ...
                                        <div class="config-provider">${((u=me.find(b=>b.id===p.provider))==null?void 0:u.label)||k.escapeHtml(p.provider)} • ${k.escapeHtml(p.model)}</div>
                                    </div>
