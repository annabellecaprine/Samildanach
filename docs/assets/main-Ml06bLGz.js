...
<body>
    <p>${e}</p>
            ...
            <div class="project-layout">
                <div class="project-content">
                    ...
                    <div class="project-header">
                        ...
                        <input id="proj-title" type="text" value="${k.escapeHtml(w.project.title)}" 
                            placeholder="Setting Title" class="project-title">
                        <div class="project-author">
                            <input id="proj-author" type="text" value="${k.escapeHtml(w.project.author)}" 
                    ...
                    <div class="project-details">
                        ...
                        <div class="details-grid">
                            <div class="detail-field">
                                ...
                                <input id="proj-version" type="text" value="${k.escapeHtml(w.project.version)}" class="input">
                            </div>
                            <div class="detail-field">
                                <label class="label">Genre</label>
                                <input id="proj-genre" type="text" value="${k.escapeHtml(w.project.genre)}" 
                        ...
                        <div class="detail-field">
                            ...
                            <input id="proj-system" type="text" value="${k.escapeHtml(w.project.system)}" 
                        ...
                        <div class="detail-field" style="margin-top: 16px;">
                            ...
                            <textarea id="proj-description" rows="4" 
                                placeholder="Describe your setting..." class="textarea">${k.escapeHtml(w.project.description)}</textarea>
                        </div>
                    </div>
        ...
        `;const a=()=>{w.updateProject({title:t.querySelector("#proj-title").value,author:t.querySelector("#proj-author").value,version:t.querySelector("#proj-version").value,genre:t.querySelector("#proj-genre").value,system:t.querySelector("#proj-system").value,description:t.querySelector("#proj-description").value})};t.querySelectorAll("input, textarea").forEach(r=>{r.oninput=a}),t.querySelector("#btn-export").onclick=async()=>{try{const r=(w.project.title||"samildanach").toLowerCase().replace(/\s+/g,"-"),l=await _.toJSON();_.download(JSON.stringify(l,null,2),`${r}.json`,"application/json"),L.show("Project exported successfully","success")}catch(r){L.show("Export failed: "+r.message,"error")}};const o=t.querySelector("#import-file");t.querySelector("#btn-import").onclick=()=>o.click(),o.onchange=async r=>{const l=r.target.files[0];if(l)try{const h=await l.text(),c=JSON.parse(h);let d={entries:0,rules:0,flows:0};if(c.meta&&w.updateProject(c.meta),c.entries&&Array.isArray(c.entries)){await C.init();for(const y of c.entries)await C.addItem(y.type,y.data);d.entries=c.entries.length}if(c.rules&&Array.isArray(c.rules)){await P.init();for(const y of c.rules)await P.add(y.type,y.data);d.rules=c.rules.length}if(c.flows&&Array.isArray(c.flows)){await T.init();for(const y of c.flows)await T.update(y);d.flows=c.flows.length}L.show(`Imported: ${d.entries} Items, ${d.rules} Rules, ${d.flows} Flows`,"success"),setTimeout(()=>location.reload(),1500)}catch(h){console.error(h),L.show("Import failed: "+h.message,"error")}}}};class A{constructor(e={}){this.title=e.title||"Modal",this.content=e.content||"",this.actions=e.actions||[],this.onClose=e.onClose||null,this.element=null}show(){this.element=document.createElement("div"),this.element.className="modal-overlay",this.element.innerHTML=`
            <div class="modal-content">
                <h3 class="modal-title">${this.title}</h3>
                <div class="modal-body"></div>
                <div class="modal-actions"></div>
            </div>
        `;const e=this.element.querySelector(".modal-body");typeof this.content=="string"?e.innerHTML=this.content:this.content instanceof HTMLElement&&e.appendChild(this.content);const i=this.element.querySelector(".modal-actions");this.actions.forEach(s=>{const n=document.createElement("button");n.className=s.className||"btn btn-secondary",n.innerText=s.label,n.onclick=()=>{s.onClick&&s.onClick(this)},i.appendChild(n)}),this.element.onclick=s=>{s.target===this.element&&this.close()},document.body.appendChild(this.element)}close(){this.element&&(this.element.remove(),this.element=null),this.onClose&&this.onClose()}getBody(){var e;return(e=this.element)==null?void 0:e.querySelector(".modal-body")}static confirm(e,i){return new Promise(s=>{const n=new A({title:e,content:`<p>${i}</p>`,actions:[{label:"Cancel",className:"btn btn-secondary",onClick:()=>{n.close(),s(!1)}},{label:"Confirm",className:"btn btn-primary",onClick:()=>{n.close(),s(!0)}}]});n.show()})}static alert(e,i){return new Promise(s=>{const n=new A({title:e,content:`<p>${i}</p>`,actions:[{label:"OK",className:"btn btn-primary",onClick:()=>{n.close(),s()}}]});n.show()})}static prompt(e,i=""){return new Promise(s=>{const n=document.createElement("div");n.innerHTML=`
                <input type="text" class="input" style="width:100%" value="${i}">
            `;const a=n.querySelector("input"),o=new A({title:e,content:n,actions:[{label:"Cancel",className:"btn btn-secondary",onClick:()=>{o.close(),s(null)}},{label:"OK",className:"btn btn-primary",onClick:()=>{o.close(),s(a.value)}}]});o.show(),setTimeout(()=>a.focus(),50),a.onkeydown=r=>{r.key==="Enter"&&(o.close(),s(a.value))}})}}class Be{constructor(e,i={}){this.container=e,this.items=[],this.activeCategory=null,this.activeItemId=null,this.onSelect=i.onSelect||null,this.onCreate=i.onCreate||null}setItems(e){this.items=e,this.render()}setActiveItem(e){this.activeItemId=e,this.renderList()}render(){this.container.innerHTML=`
        ...
        `;const i=this.container.querySelector("#asset-title"),s=this.container.querySelector("#metadata-fields"),n=this.container.querySelector("#asset-rte-mount");this.container.querySelector("#save-status"),e.fields.forEach(l=>{const h=document.createElement("div");h.className="metadata-field";const c=document.createElement("label");c.innerText=l.label,c.className="label";let d;l.type==="textarea"?(d=document.createElement("textarea"),d.rows=2,d.className="textarea"):(d=document.createElement("input"),d.type="text",d.className="input"),d.value=this.item.data[l.key]||"",d.oninput=()=>{this.item.data[l.key]=d.value,this.save()},h.appendChild(c),h.appendChild(d),s.appendChild(h)});const a=new De(n,this.item.data.description||"",{getEntries:this.getEntries,onLinkClick:this.onLinkClick});a.render(),this.editorInstance=a,a.onChange=l=>{this.item.data.description=l,this.save()};let o=null;i.oninput=()=>{this.item.data.name=i.value,this.save(),clearTimeout(o),o=setTimeout(()=>{this.onNameChange&&this.onNameChange(this.item)},300)};const r=this.container.querySelector("#btn-delete-entry");r&&(r.onclick=async()=>{if(await A.confirm("Delete Entry",`Are you sure you want to delete "${this.item.data.name||"this entry"}"?`)&&this.onDelete)try{await this.onDelete(this.item),L.show("Entry deleted","success")}catch{L.show("Failed to delete entry","error")}})}async save(){const e=this.container.querySelector("#save-status");e&&(e.innerText="Saving..."),this.onSave&&await this.onSave(this.item),e&&(e.innerText="Saved")}}class We{constructor(e,i={}){this.container=e,this.item=null,this.allItems=[],this.onSave=i.onSave||null,this.onNavigate=i.onNavigate||null}setItem(e,i){this.item=e,this.allItems=i,e&&this.render()}render(){this.item&&(this.item.data.relationships||(this.item.data.relationships=[]),this.container.innerHTML=`
        ...
        `;const i=t.querySelector("#lib-sidebar"),s=t.querySelector("#lib-editor-area"),n=t.querySelector("#lib-relationships-area");if(x.allItems=await C.list(),x.entryList=new Be(i,{onSelect:a=>x.selectItem(a,s,n),onCreate:()=>x.showCreateModal()}),x.entryList.setItems(x.allItems),x.entryEditor=new Ge(s,{onSave:async a=>{await C.updateItem(a)},onNameChange:a=>{x.entryList.setItems(x.allItems)},onLinkClick:a=>{const o=x.allItems.find(r=>(r.data.name||"").toLowerCase()===a.toLowerCase());o&&x.selectItem(o,s,n)},onDelete:async a=>{await C.deleteItem(a.id),x.allItems=x.allItems.filter(o=>o.id!==a.id),x.entryList.setItems(x.allItems),x.entryEditor.showEmpty(),n.style.display="none",w.updateSession({activeEntryId:null})},getEntries:()=>x.allItems}),x.entryEditor.showEmpty(),x.relationshipManager=new We(n,{onSave:async a=>{await C.updateItem(a)},onNavigate:a=>{x.selectItem(a,s,n)}}),w.session.activeEntryId){const a=x.allItems.find(o=>o.id===w.session.activeEntryId);a&&x.selectItem(a,s,n)}},selectItem(t,e,i){x.activeItem=t,x.entryList.setActiveItem(t.id),x.entryEditor.setItem(t),i.style.display="block",x.relationshipManager.setItem(t,x.allItems),w.updateSession({activeEntryId:t.id})},showCreateModal(){const t=document.createElement("div");t.className="grid-3",ee().forEach(i=>{const s=document.createElement("button");s.className="btn btn-secondary",s.style.cssText="flex-direction:column; padding:12px;",s.innerHTML=`<span style="font-size:20px;">${i.icon}</span><span class="text-xs">${i.label}</span>`,s.onclick=async()=>{e.close();const n=await C.addItem(i.id,{name:`New ${i.label}`,description:""});x.allItems.push(n),x.entryList.setItems(x.allItems);const a=document.querySelector("#lib-editor-area"),o=document.querySelector("#lib-relationships-area");x.selectItem(n,a,o)},t.appendChild(s)});const e=new A({title:"Create New Entry",content:t,actions:[{label:"Cancel",className:"btn btn-secondary",onClick:i=>i.close()}]});e.show()}},H={id:"grimoire",label:"Grimoire",icon:"📖",_state:{activeCategory:"item",selectedRuleId:null,searchQuery:""},render:async t=>{var h;await P.init();let e=await P.list();const i=Ne();let s=H._state.activeCategory||((h=i[0])==null?void 0:h.id)||"item",n=H._state.selectedRuleId,a=n?e.find(c=>c.id===n):null,o=H._state.searchQuery||"",r=null;async function l(){var S,m,p;e=await P.list(),a=n?e.find(u=>u.id===n):null;const c=Pe(s);let d=e.filter(u=>u.type===s);if(o){const u=o.toLowerCase();d=d.filter(b=>(b.data.name||"").toLowerCase().includes(u)||(b.data.description||"").toLowerCase().includes(u)||Object.values(b.data).some(f=>typeof f=="string"&&f.toLowerCase().includes(u)))}t.innerHTML=`
            ...
            `,t.querySelectorAll(".cat-tab").forEach(u=>{u.onclick=()=>{s=u.dataset.cat,n=null,a=null,o="",H._state.activeCategory=s,H._state.selectedRuleId=null,H._state.searchQuery="",l()}});const y=t.querySelector("#grimoire-search");if(y&&(y.oninput=u=>{o=u.target.value,H._state.searchQuery=o,l().then(()=>{const b=t.querySelector("#grimoire-search");b&&(b.focus(),b.setSelectionRange(b.value.length,b.value.length))})}),t.querySelectorAll(".rule-item").forEach(u=>{u.onclick=()=>{n=u.dataset.id,a=e.find(b=>b.id===n),H._state.selectedRuleId=n,l()}}),(S=t.querySelector("#btn-add-rule"))==null||S.addEventListener("click",async()=>{const u=await P.add(s,{name:`New ${(c==null?void 0:c.label)||"Rule"}`,description:""});console.log("[Grimoire] Created rule:",u.id),n=u.id,H._state.selectedRuleId=n,o="",H._state.searchQuery="",l()}),(m=t.querySelector("#btn-delete-rule"))==null||m.addEventListener("click",async()=>{!a||!await A.confirm("Delete Rule",`Delete "${a.data.name||"this rule"}"?`)||(await P.delete(a.id),L.show("Rule deleted","success"),n=null,a=null,H._state.selectedRuleId=null,l())}),(p=t.querySelector("#btn-save-rule"))==null||p.addEventListener("click",async()=>{var f;if(!a)return;const b={name:((f=t.querySelector("#rule-name"))==null?void 0:f.value)||""};t.querySelectorAll(".field-input").forEach(g=>{b[g.dataset.field]=g.value}),r&&(b.description=r.getValue()),a.data=b,await P.update(a),L.show("Rule saved","success"),l()}),a){const u=t.querySelector("#description-editor");u&&(r=new De(u,a.data.description||""),r.render())}}await l()}},Ee={id:"graph",label:"Graph",icon:"🕸️",render:async(t,e)=>{try{await C.init()}catch(f){t.innerHTML=`<div class="text-muted">Vault Error: ${f.message}</div>`;return}t.style.padding="0",t.innerHTML=`
        ...
        `;const i=t.querySelector("#lab-expr"),s=t.querySelector("#lab-roll"),n=t.querySelector("#lab-result"),a=t.querySelector("#lab-stats");i.oninput=()=>{const p=ve.stats(i.value);p?a.innerText=`Range: ${p.min}–${p.max} | Average: ${p.average}`:a.innerText=""},i.oninput(),s.onclick=()=>{const p=i.value.trim();if(!p)return;const u=ve.roll(p);u.error?n.innerHTML=`<span style="color:var(--status-error);">${k.escapeHtml(u.error)}</span>`:n.innerHTML=`
                ...
                `},i.onkeydown=p=>{p.key==="Enter"&&s.onclick()};const o=t.querySelector("#sim-expr"),r=t.querySelector("#sim-iterations"),l=t.querySelector("#sim-run"),h=t.querySelector("#sim-results"),c=t.querySelector("#histogram");l.onclick=()=>{const p=o.value.trim(),u=parseInt(r.value);p&&(l.disabled=!0,l.innerText="Running...",setTimeout(()=>{const b=pe.runDiceSimulation(p,u);if(l.disabled=!1,l.innerText="Run",b.error){h.style.display="none",L.show(b.error,"error");return}h.style.display="block",t.querySelector("#stat-min").innerText=b.stats.min,t.querySelector("#stat-max").innerText=b.stats.max,t.querySelector("#stat-mean").innerText=b.stats.mean,t.querySelector("#stat-median").innerText=b.stats.median,t.querySelector("#stat-mode").innerText=b.stats.mode,t.querySelector("#stat-stddev").innerText=b.stats.stdDev;const f=pe.getHistogramData(b),g=Math.max(...f.map(v=>v.count));c.innerHTML=f.map(v=>`
...
[${(b==null?void 0:b.label)||u.type}] ${u.data.name||"Untitled"}`,u.data.description){const f=u.data.description.replace(/<[^>]+>/g,"").substring(0,300);m+=`: ${f}`}b!=null&&b.fields&&b.fields.forEach(f=>{u.data[f.key]&&(m+=` | ${f.label}: ${u.data[f.key]}`)}),u.data.relationships&&u.data.relationships.length>0&&(m+=`
  Relationships:`,u.data.relationships.forEach(f=>{const g=s.find(E=>E.id===f.targetId),v=(g==null?void 0:g.data.name)||"(Unknown)",$=f.type||"related to";m+=`
    - ${$}: ${v}`,f.notes&&(m+=` (${f.notes})`)}))}),m+=`
[END CONTEXT]`)}return m}async function S(){const m=o.value.trim();if(!m)return;const p={id:ne(),role:"user",content:m,timestamp:new Date().toISOString()};e.history.push(p),o.value="",d(),i(),r.disabled=!0,r.textContent="Thinking...";try{if(!G.getActiveConfig())throw new Error("No API configuration. Go to Settings to add one.");const b=y(),g=e.history.map(E=>({role:E.role==="user"?"user":"model",content:E.content})).slice(-20),v=await G.generate(b,g),$={id:ne(),role:"model",content:v,timestamp:new Date().toISOString()};e.history.push($),i(),d()}catch(u){console.error("[Scribe]",u),e.history.push({id:ne(),role:"model",content:`[Error: ${u.message}]`,timestamp:new Date().toISOString()}),d()}finally{r.disabled=!1,r.textContent="Send"}}r.onclick=S,o.onkeydown=m=>{m.key==="Enter"&&!m.shiftKey&&(m.preventDefault(),S())},t.querySelector("#btn-clear").onclick=async()=>{await A.confirm("Clear Chat","Clear all messages?")&&(e.history=[],i(),d(),L.show("Chat cleared","success"))},t.querySelector("#btn-save-session").onclick=async()=>{const m=await A.prompt("Session name:",`Session ${Object.keys(e.sessions).length+1}`);m&&(e.sessions[m]={history:[...e.history],mode:e.mode,selectedEntries:[...e.selectedEntries],savedAt:new Date().toISOString()},i(),h.innerHTML='<option value="">-- Sessions --</option>'+Object.keys(e.sessions).map(p=>`<option value="${p}">${p}</option>`).join(""),L.show("Session saved","success"))},t.querySelector("#btn-load-session").onclick=()=>{const m=h.value;if(!m||!e.sessions[m])return;const p=e.sessions[m];e.history=[...p.history],e.mode=p.mode,e.selectedEntries=[...p.selectedEntries],i(),ge.render(t)}}},Me={id:"export",label:"Export",icon:"📤",render:(t,e)=>{t.innerHTML=`
                            ...
                            `:i.map(p=>{var u;return`
                                <div class="config-card ${p.id===(s==null?void 0:s.id)?"active":""}" data-id="${p.id}">
                                    <div class="config-info">
                                        <div class="config-name">${Utils.escapeHtml(p.name||"Unnamed")}</div>
                                        <div class="config-provider">${((u=me.find(b=>b.id===p.provider))==null?void 0:u.label)||Utils.escapeHtml(p.provider)} • ${Utils.escapeHtml(p.model)}</div>
                                    </div>
